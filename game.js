...
const CONFIG = {
  ...
  PLAYER_DRAW_H: 132, CAM_FOLLOW: 0.60, CAM_LERP: 9,   // 新スプライト(余白込みキャンバス)に合わせ拡大
};
...
const SPRITE_VER = 8;   // スプライト差し替え時にbump＝ブラウザ画像キャッシュ回避
...
function drawPlayer() {
  ...
  if (img && img.ok) { const dh = CONFIG.PLAYER_DRAW_H, dw = dh * (img.width / img.height); ctx.drawImage(img, x - dw / 2, y + p.h / 2 - dh, dw, dh); }   // 左右はスプライト側に内包＝反転しない
