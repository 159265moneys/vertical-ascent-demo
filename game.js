...
const CONFIG = {
  ...
  GOLD: { target: 1, obstacle: 2, floater: 2, attacker: 4, crawler: 1, turret: 2, assassin: 3, rock: 2 },
  SP: { target: 1, obstacle: 1, floater: 1, attacker: 2, crawler: 1, turret: 2, assassin: 2, rock: 1 },
  TELEGRAPH_LEAD: 0.5,
  // --- 横穴ハブ（鍵・器）---
...
// 高度で敵を1種ずつ導入＝難易度傾斜
function rosterAt(m) { return m < 25 ? ['target', 'crawler', 'turret', 'rock'] : m < 60 ? ['target', 'crawler', 'turret', 'rock', 'assassin', 'obstacle'] : m < 110 ? ['target', 'crawler', 'turret', 'rock', 'assassin', 'obstacle', 'floater'] : ['target', 'obstacle', 'floater', 'crawler', 'turret', 'rock', 'assassin', 'attacker', 'attacker']; }
...
const SPRITE_VER = 18;   // スプライト差し替え時にbump＝ブラウザ画像キャッシュ回避
...
function drawEnemy(e) {
  ...
  if (e.dead) {   // 死体：死亡コマを回転させながら落下
    ...
    const corpse = e.type === 'crawler' ? e01r[4] : e.type === 'turret' ? e06[6] : e.type === 'assassin' ? e03d[3] : e.type === 'rock' ? e10d[(e.deadT || 0) < 0.25 ? 1 : 2] : null;   // crawler=崩れ/turret=萎れ/assassin=死亡#4/rock=割れ→粉砕(2,3)
    if (corpse && corpse.ok) { const dh = e.type === 'turret' ? CONFIG.TURRET_DRAW_H : e.type === 'assassin' ? CONFIG.ASN_DRAW_H : e.type === 'rock' ? CONFIG.ROCK_DRAW_H : CONFIG.CRAWL_DRAW_H, dw = dh * corpse.width / corpse.height; ctx.drawImage(corpse, -dw / 2, -dh / 2, dw, dh); }
