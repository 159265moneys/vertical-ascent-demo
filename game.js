...
let meta = Object.assign({
  ...
  unlocked: ['dash', 'shun', 'mayu', 'kenpa', 'spin', 'homura'], slots: { W: 'dash', A: 'kenpa', S: 'shun', D: 'homura' },
  ownedCharms: [], equippedCharms: [], notchMax: 3,
}, loadMeta());
for (const id of ['dash', 'shun']) if (!meta.unlocked.includes(id)) meta.unlocked.push(id);       // 突撃/瞬影は既存セーブにも解放
...
const CASTERS = { kenpa: castKenpa, homura: castHomura, spin: castSpin, mayu: castMayu, raijin: castRaijin, tri: castTri, dash: castDash, shun: castShun };   // 疾駆は枠スキルでなく専用キー(ろ/RB)から castShikku を直接呼ぶ
