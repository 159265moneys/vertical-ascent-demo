...
const CONFIG = {
  ...
  PLAYER_DRAW_H: 162, SPRITE_FEET_FRAC: 0.764, CAM_FOLLOW: 0.60, CAM_LERP: 9,   // 足基準スプライト：FEET_FRAC=上端から足までの割合(描画で足を接地に合わせる)
};
...
const SPRITE_VER = 9;   // スプライト差し替え時にbump＝ブラウザ画像キャッシュ回避
...
function drawPlayer() {
  ...
  if (img && img.ok) { const dh = CONFIG.PLAYER_DRAW_H, dw = dh * (img.width / img.height); ctx.drawImage(img, x - dw / 2, y + p.h / 2 - dh * CONFIG.SPRITE_FEET_FRAC, dw, dh); }   // 足基準で接地＝武器が上下左右に出ても体は不動／左右はスプライト内包
