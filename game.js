...
addEventListener('keydown', e => {
  ...
  if (e.code === 'KeyM' && player && !player.bombCharging && player.bombCd <= 0 && Math.floor(player.ap) >= CONFIG.BOMB_AP) { player.bombCharging = true; player.bombCharge = 0; }   // ポゴ爆弾チャージ開始(離すと投擲)
...
addEventListener('keyup', e => { keys[e.code] = false; if (e.code === 'KeyM' && player && player.bombCharging) { throwBomb(player.bombCharge); player.bombCharging = false; } });   // 離した瞬間に投擲(押下時間で飛距離)
