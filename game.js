...
const CONFIG = {
  ...
  GROUND_JUMP_VY: -1320,
  COYOTE: 0.09, JUMP_BUFFER: 0.10, JUMP_CUT: 0.45,   // JUMP_CUT=ジャンプ/壁キック上昇中にボタンを離すと上昇速度を×この値(短押し=ミニジャンプ/長押し=最大高度)
  CLING_GRIP_TIME: 3.0,
  ...
  SPIN_AP: 2, SPIN_MULT: 1.2, SPIN_R: 72, SPIN_ACTIVE: 0.24, SPIN_CD: 0.40,        // S：回転斬り（円の内側=全部AoE・円周は小さめ・回転が見える長さ）
...
let chainDemo = -1, chainDemoT = 0;   // デバッグ：チェイン連続デモ(AP無視で既定スキルを順に発火)。stepindex/次発火までの残り時間
const skillEdge = { W: false, A: false, S: false, D: false };
const pad = { left: false, right: false, up: false, down: false, rawLeft: false, rawRight: false, lb: false, jump: false };   // ゲームパッド由来の連続入力(pollGamepadが毎フレーム更新)
...
addEventListener('keydown', e => {
  ...
  if (e.code === 'IntlRo') castShikku();   // 疾駆(ダッシュ)＝ろキー。コア能力(スキル枠外)
...
function pollGamepad() {
  const gp = activePad();
  if (!gp) { pad.left = pad.right = pad.up = pad.down = pad.rawLeft = pad.rawRight = pad.lb = pad.jump = false; padPrev = {}; return; }
  const b = i => !!(gp.buttons[i] && gp.buttons[i].pressed), ax = i => gp.axes[i] || 0, DZ = 0.4;
  const sx = ax(0), sy = ax(1), lb = b(4);
  pad.lb = lb; pad.jump = b(0);                                                           // LB=スキル修飾(押し続け)／A保持=可変ジャンプ用
  ...
  const cur = { A: b(0), B: b(1), X: b(2), Y: b(3), RB: b(5), RT: b(7), start: b(9), dU: b(12), dD: b(13) };
  ...
  if (inHideout) { if (e('dU')) hideoutKey('ArrowUp'); if (e('dD')) hideoutKey('ArrowDown'); if (e('A')) hideoutKey('Enter'); if (e('B')) hideoutKey('Escape'); }   // メニュー操作
    ...
    if (e('RB')) castShikku();                                                            // RB=疾駆(ダッシュ・コア能力)
...
function reset() {
  player = {
    ...
    spinTimer: 0, spinCd: 0, spinHit: null, kenpaCd: 0, homuraCd: 0, mayuCd: 0, mayuTimer: 0, raijinCd: 0, triCd: 0, dashCd: 0, dashTimer: 0, shikkuCd: 0, shunCd: 0, shunT: 0, shunHit: false, shunVanish: 0, tenshoCd: 0, tenshoT: 0, tenshoPhase: 'dashup', tenshoSlashLeft: 0, tenshoSlashT: 0, tenshoSlashIdx: 0, tenshoSlashAng: 0, tenshoSlashPose: 'atk', bombCd: 0, bombCharging: false, bombCharge: 0, jumpHeld: false, reclingLock: 0,
...
function damage(q, knockY, knockX) {
  ...
  player.jumpHeld = false;   // 被弾はジャンプ追跡を打ち切り(KBが上昇カットされないように)
...
// 三日月の軌跡＝円弧の背骨に沿って太さenvelopeで内外オフセットしたリボンを塗る(swoosh)。screen座標
function inkCrescent(cx, cy, r, a0, sweep, wMax, color, env) {
  env = env || INK_ENV.crescent;
  const N = 30;
  ctx.beginPath();
  for (let i = 0; i <= N; i++) { const t = i / N, a = a0 + sweep * t, w = wMax * env(t) / 2, ro = r + w, x = cx + Math.cos(a) * ro, y = cy + Math.sin(a) * ro; if (i) ctx.lineTo(x, y); else ctx.moveTo(x, y); }
...
function castDash() { const p = player; if (p.dashCd > 0 || Math.floor(p.ap) < CONFIG.DASH_AP) return;
  ...
  p.dashVx = dx / dist * speed; p.dashVy = dy / dist * speed; p.dashTimer = CONFIG.DASH_ACTIVE; p.dashHit = new Set(); p.dashIfr = true; p.facing = dx < 0 ? -1 : 1; p.state = 'dash'; p.clingWall = 0; }
function castShikku() { const p = player; if (!p || p.state === 'fallStun' || p.shikkuCd > 0 || Math.floor(p.ap) < CONFIG.SHIKKU_AP) return; p.ap -= CONFIG.SHIKKU_AP; p.shikkuCd = CONFIG.SHIKKU_CD;
...
const TENSHO_FAN = [{ off: 0.5, pose: 'up' }, { off: -0.5, pose: 'pogo' }, { off: 0, pose: 'atk' }];   // 向いてる方の扇：上↗→下↘→横→(斬りごとに角度もスプライトも変える)
function tenshoSlash() { const p = player; const f = TENSHO_FAN[p.tenshoSlashIdx % TENSHO_FAN.length]; p.tenshoSlashIdx++;
  const base = p.facing > 0 ? 0 : Math.PI; p.tenshoSlashAng = base - p.facing * f.off; p.tenshoSlashPose = f.pose;   // facing側で上下に振る(off>0=上)
  p.nailTimer = CONFIG.NAIL_ACTIVE * 0.7; p.nailHitThisSwing = true;   // 自前判定なので通常ヒット抑止
  for (const e of enemies) if (e.alive && !e.dead && !e.gdeath && Math.hypot(e.x - p.x, e.y - p.y) < CONFIG.TENSHO_SLASH_R + e.w / 2) hitEnemy(e, CONFIG.ATK_BASE * CONFIG.TENSHO_SLASH_MULT, p.tenshoSlashAng);
...
function castTensho() { const p = player; if (!p || p.state === 'fallStun' || p.tenshoCd > 0 || Math.floor(p.ap) < CONFIG.TENSHO_AP) return; p.ap -= CONFIG.TENSHO_AP; p.tenshoCd = CONFIG.TENSHO_CD;
  ...
  const tlv = skillLv('tensho'); if (tlv >= 2) { p.state = 'tensho'; p.tenshoPhase = 'dashup'; p.tenshoT = CONFIG.TENSHO_DASH_T; p.tenshoSlashLeft = tlv >= 3 ? 3 : 1; p.tenshoSlashT = 0; p.tenshoSlashIdx = 0; p.clingWall = 0; p.iframe = Math.max(p.iframe, 0.1); }   // Lv2+：上へ高速ダッシュ→その場斬り(Lv1は打ち上げのみ・位置不変)
}
...
function update(dt) {
  ...
  for (const k of ['pogoCd','upCd','nailCd','spinCd','kenpaCd','homuraCd','mayuCd','raijinCd','triCd','dashCd','shikkuCd','shunCd','tenshoCd','bombCd','reclingLock','pogoTimer','upTimer','nailTimer','spinTimer','mayuTimer']) if (p[k] > 0) p[k] -= dt;
  ...
  if (chainDemo >= 0) { chainDemoT -= dt; if (chainDemoT <= 0) { if (chainDemo < CHAIN_DEMO.length) { const s = CHAIN_DEMO[chainDemo]; p.ap = 99; s.f(); chainDemo++; chainDemoT = s.g; } else chainDemo = -1; } }   // チェインデモ：各stepの間隔gで次へ(AP無視)
  ...
  for (const e of enemies) {
    ...
    if (e.launched > 0) { e.launched -= dt; e.y = Math.max(e.launchTargetY, e.y - CONFIG.TENSHO_LIFT_V * dt); if (e.launched <= 0 || e.y <= e.launchTargetY) e.launched = 0; continue; }   // 天衝の打ち上げ中＝通常AI停止・真上へ規定距離まで
...
function spriteKey() {
  ...
  else if (p.state === 'tensho') act = (p.tenshoPhase === 'slash' ? p.tenshoSlashPose : 'up');   // 天衝：ダッシュ中=上/その場斬り=斬りごとに上↗下↘横→でポーズ切替
...
function render() {
  ...
  if (p.nailTimer > 0) { if (p.state === 'tensho' && p.tenshoPhase === 'slash') { const pr = 1 - p.nailTimer / (CONFIG.NAIL_ACTIVE * 0.7), aa = pr < 0.6 ? 1 : Math.max(0, 1 - (pr - 0.6) / 0.4), gg = 0.85 + Math.min(1, pr / 0.25) * 0.15; slashArc(p.x, sy(p.y), p.tenshoSlashAng, aa, gg, slashReach); }   // 天衝その場斬り＝扇角度で三日月(向いてる方の上↗下↘横→)
    else drawSlash(p.x + p.facing * p.w / 2, sy(p.y), 1 - p.nailTimer / CONFIG.NAIL_ACTIVE, p.facing < 0 ? 'left' : 'right'); }   // 前＝凸を進行方向へ
