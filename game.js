...
const SKILL_META = {
  ...
  shikku: { name: '疾駆', ap: 'SHIKKU_AP', cd: 'shikkuCd', cdMax: 'SHIKKU_CD' },
};
...
let meta = Object.assign({
  ...
  unlocked: ['dash', 'shikku', 'mayu', 'kenpa', 'spin', 'homura'], slots: { W: 'dash', A: 'shikku', S: 'spin', D: 'homura' },
  ownedCharms: [], equippedCharms: [], notchMax: 3,
}, loadMeta());
for (const id of ['dash', 'shikku']) if (!meta.unlocked.includes(id)) meta.unlocked.push(id);     // 新スキルは既存セーブにも解放
if (!meta.fixShikku) { if (!Object.values(meta.slots).includes('shikku')) meta.slots.A = 'shikku'; meta.fixShikku = true; saveMeta(); }   // 疾駆をA枠へ(検証用・1回だけ)
...
function reset() {
  player = {
    ...
    spinTimer: 0, spinCd: 0, kenpaCd: 0, homuraCd: 0, mayuCd: 0, mayuTimer: 0, raijinCd: 0, triCd: 0, dashCd: 0, dashTimer: 0, shikkuCd: 0, bombCd: 0, bombCharging: false, bombCharge: 0,
...
function castShikku() { const p = player; if (p.shikkuCd > 0 || Math.floor(p.ap) < CONFIG.SHIKKU_AP) return; p.ap -= CONFIG.SHIKKU_AP; p.shikkuCd = CONFIG.SHIKKU_CD;
...
const CASTERS = { kenpa: castKenpa, homura: castHomura, spin: castSpin, mayu: castMayu, raijin: castRaijin, tri: castTri, dash: castDash, shikku: castShikku };
