...
const CONFIG = {
  ...
  TENSHO_AP: 3, TENSHO_CD: 0.6, TENSHO_RANGE: 110, TENSHO_MULT: 1.4, TENSHO_LAUNCH_DIST: 180, TENSHO_LAUNCH_T: 0.3, TENSHO_LIFT_V: 600,   // 天衝：範囲内の敵を真上へ規定距離打ち上げ(斬り上げ)
  TENSHO_RISE_V: 600, TENSHO_RISE_T: 0.3, TENSHO_SLASH_R: 92, TENSHO_SLASH_MULT: 1.3, TENSHO_SLASH_GAP: 0.09,   // Lv2+：打ち上げに追従上昇しながら斬りつけ(Lv2=1回/Lv3=3回)
...
function reset() {
  player = {
    ...
    spinTimer: 0, spinCd: 0, spinHit: null, kenpaCd: 0, homuraCd: 0, mayuCd: 0, mayuTimer: 0, raijinCd: 0, triCd: 0, dashCd: 0, dashTimer: 0, shikkuCd: 0, shunCd: 0, shunT: 0, shunHit: false, shunVanish: 0, tenshoCd: 0, tenshoT: 0, tenshoSlashes: 0, tenshoSlashT: 0, bombCd: 0, bombCharging: false, bombCharge: 0, jumpHeld: false, reclingLock: 0,
...
function castTensho() { const p = player; if (!p || p.state === 'fallStun' || p.tenshoCd > 0 || Math.floor(p.ap) < CONFIG.TENSHO_AP) return; p.ap -= CONFIG.TENSHO_AP; p.tenshoCd = CONFIG.TENSHO_CD;
  let launched = 0; for (const e of enemies) { if (!e.alive || e.dead || e.gdeath) continue; if (Math.hypot(e.x - p.x, e.y - p.y) > CONFIG.TENSHO_RANGE + e.w / 2) continue;
    hitEnemy(e, CONFIG.ATK_BASE * CONFIG.TENSHO_MULT, -Math.PI / 2); e.launched = CONFIG.TENSHO_LAUNCH_T; e.launchTargetY = e.y - CONFIG.TENSHO_LAUNCH_DIST; launched++; }   // 範囲内の敵を斬り上げ→真上へ規定距離打ち上げ
  shake = Math.max(shake, 7);
  if (tenshoLv >= 2 && launched > 0) { p.state = 'tensho'; p.tenshoT = CONFIG.TENSHO_RISE_T; p.tenshoSlashes = tenshoLv >= 3 ? 3 : 1; p.tenshoSlashT = 0; p.clingWall = 0; p.iframe = Math.max(p.iframe, CONFIG.TENSHO_RISE_T + 0.1); }   // Lv2+：追従上昇して斬りつけ
  else p.vy = Math.min(p.vy, -260); }   // Lv1：自分は軽く浮くだけ
...
function update(dt) {
  ...
  if (p.state === 'tensho') {   // 天衝Lv2+：打ち上げに追従して上昇しながら斬りつけ
    p.tenshoT -= dt; p.vy = -CONFIG.TENSHO_RISE_V; p.vx *= 0.85; p.iframe = Math.max(p.iframe, 0.08);
    p.tenshoSlashT -= dt;
    if (p.tenshoSlashes > 0 && p.tenshoSlashT <= 0) { p.tenshoSlashes--; p.tenshoSlashT = CONFIG.TENSHO_SLASH_GAP;
      for (const e of enemies) if (e.alive && !e.dead && !e.gdeath && Math.hypot(e.x - p.x, e.y - p.y) < CONFIG.TENSHO_SLASH_R + e.w / 2) hitEnemy(e, CONFIG.ATK_BASE * CONFIG.TENSHO_SLASH_MULT, -Math.PI / 2 + (Math.random() - 0.5));
      shake = Math.max(shake, 5);
    }
    if (p.tenshoT <= 0) { p.state = 'air'; p.vy = -240; p.coyote = 0; }   // 終了＝軽い余韻上昇
