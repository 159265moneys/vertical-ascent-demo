...
// デバッグ：チェイン連続デモ。既定スキルを順に発火(CT/APは直前に解除＝計算外の動きは出さず"今定義してる範囲"で繋がるか確認)
const CHAIN_DEMO = [
  () => { const p = player; if (p.grounded || p.coyote > 0) jumpBuffer = CONFIG.JUMP_BUFFER; else p.vy = -CONFIG.GROUND_JUMP_VY * 0 - 900; },   // ①跳ぶ(空中なら上昇付与)
  () => { player.tenshoCd = 0; castTensho(); },                                            // ②天衝(打ち上げ＋追従)
  () => { player.shunCd = 0; castShun(); },                                                // ③瞬影
  () => { const p = player; p.pogoCd = 0; p.pogoTimer = CONFIG.POGO_ACTIVE; p.pogoHitThisSwing = false; },   // ④ポゴ
  () => { player.spinCd = 0; castSpin(); },                                                // ⑤回転斬り
  () => { player.shikkuCd = 0; castShikku(); },                                            // ⑥疾駆
  () => { player.dashCd = 0; castDash(); },                                                // ⑦突撃
  () => { const p = player; p.bombCd = 0; throwBomb(0); p.pogoCd = 0; p.pogoTimer = CONFIG.POGO_ACTIVE; p.pogoHitThisSwing = false; },   // ⑧爆弾真下→ポゴ起爆
];
...
function update(dt) {
  ...
  if (chainDemo >= 0) { chainDemoT -= dt; if (chainDemoT <= 0) { if (chainDemo < CHAIN_DEMO.length) { p.ap = 99; CHAIN_DEMO[chainDemo](); chainDemo++; chainDemoT = 0.4; } else chainDemo = -1; } }   // チェインデモ：0.4s毎に次スキル(AP無視)
